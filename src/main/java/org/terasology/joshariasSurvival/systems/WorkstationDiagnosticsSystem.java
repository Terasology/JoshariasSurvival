...
public class WorkstationDiagnosticsSystem extends BaseComponentSystem {
    ...
    private int getMaxDepth(ResourceUrn input, Multimap<ResourceUrn, List<ResourceUrn>> relatedInputs,
                            List<ResourceUrn> alreadyVisited) {
        int depth = 0;
        ...
        for (ResourceUrn childInput :
                relatedInputs.get(input).stream().flatMap(Collection::stream).collect(Collectors.toList())) {
    ...
    private int getMaxComplexity(ResourceUrn input, Multimap<ResourceUrn, List<ResourceUrn>> relatedInputs,
                                 List<ResourceUrn> alreadyVisited) {
        int complexity = 0;
        if (!relatedInputs.containsKey(input) || alreadyVisited.contains(input)) {
            return 1;
